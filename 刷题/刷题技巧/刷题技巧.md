...
```java
// 因为环的位置可能在链表的任何位置
// 所以设置快慢引用, 快的每次走 两格, 慢的走 一格
// 如果存在环, 则快引用 一定可以追上 慢引用
// 否则快引用就到了 null
...
+ **原理**
  ...
  > 1. 快引用 `任意时刻` 的路程是 `慢引用的两倍`
  >
  > 2. 所以**第一次相遇**的时候, 假设慢引用走了 K 步
  >    则快引用走了 2K 步
  >
  > 3. 则 `K 为环的长度的整数倍`
  >
  ...
  >
  > 5. 将`慢引用指向链表头部`, 两个引用 `同速前进`, 经过 `k - m` 的距离之后, 两者`在环的起点相遇`.
