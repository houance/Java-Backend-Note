...
> 1. 四个锁都是针对 `Synchronized` 关键词的, 并且膨胀过程`只能从左往右`.
>
> 2. 无锁 == 自旋锁
>
> 3. **偏向锁**是指, 默认自始至终`只有一个线程`在使用共享资源, 即不会去锁任何的资源, 也就没有 **阻塞和唤醒** 等动作的发生. 但是, 一旦有**其他线程竞争**, 就立即膨胀为 `轻量级锁`
>
...
> 3. **monitor 对象** 包含的信息 : 
>
>    > 1. 该对象**现在的状态**( 被线程持有 / 没有被持有 )
>    > 2. 该对象**被抢占的频繁程度**( 一定程度上用来表示`线程等待队列`的长度 )
